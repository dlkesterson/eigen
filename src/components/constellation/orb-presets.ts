...
/**
 * Deterministic visual profile generated from device ID
 * Ensures consistent appearance across sessions
 */
export interface DeviceOrbProfile {
  primaryHue: number;
  secondaryHue: number;
  pulseSpeed: number;
...
/**
 * Generate consistent orb characteristics from device ID
 * Uses deterministic hashing so the same device always looks the same
 */
export function generateOrbProfile(deviceId: string): DeviceOrbProfile {
  let hash = 0;
  ...
  return {
    ...
    secondaryHue: Math.abs((hash * 1.618) % 360), // Golden ratio offset
    pulseSpeed: 0.5 + rand(hash) * 1.5,
    rotationSpeed: 0.1 + rand(hash * 2) * 0.3,
    layerCount: 2 + Math.floor(rand(hash * 3) * 3), // 2-4 layers
    glowIntensity: 0.3 + rand(hash * 4) * 0.4,
  };
}
