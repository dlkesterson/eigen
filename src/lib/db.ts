...
/**
 * Get count of all embeddings (efficient - doesn't load data into memory)
 */
 ...
 * WARNING: This loads all embeddings into memory. For large datasets, consider pagination.
 */
...
/**
 * Get count of files without embeddings (efficient)
 */
export async function getFilesWithoutEmbeddingsCount(): Promise<number> {
    const db = await getDB();
    const fileCount = await db.count('files');
    const embeddingCount = await db.count('embeddings');
    return Math.max(0, fileCount - embeddingCount);
}

/**
 * Get files without embeddings
 * WARNING: This loads all files into memory. For large datasets, use pagination.
 */
