...
import { getActivityForPath, getMostAccessedFiles, logActivity, type ActivityLogEntry } from './db';
...
export function shouldBoostPriority(
    ...
    return patterns.some((p) => {
        ...
        return (
            ...
            (p.hour === currentHour || p.hour === currentHour + 1)
        );
    });
}
...
export async function getFolderPredictions(
    ...
    for (const { path, count } of mostAccessed) {
        ...
        const significantPatterns = patterns.filter(
            (p) => p.frequency >= config.minFrequencyThreshold
        );
...
class PredictiveSyncService {
     ...
     */
    registerRescanCallback(
        callback: RescanCallback,
