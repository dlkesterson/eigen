...
export const mockErrors = {
    network: () => new NetworkError('Network request failed', {
        context: { endpoint: 'http://localhost:8384' },
    }),

    api404: () => new APIError('Resource not found', 404, {
        context: { resource: '/rest/config' },
    }),

    api500: () => new APIError('Internal server error', 500, {
        context: { resource: '/rest/config' },
    }),

    syncthingConnection: () => new SyncthingError(
        'Connection refused',
        'CONNECTION_REFUSED',
        { context: { host: '127.0.0.1', port: 8384 } }
    ),

    syncthingTimeout: () => new SyncthingError(
        'Request timed out',
        'TIMEOUT',
        { context: { command: 'get_system_status', timeout: 10000 } }
    ),

    validation: (fields?: Record<string, string[]>) => new ValidationError(
        'Validation failed',
        fields || { folderId: ['Folder ID is required'] }
    ),

    config: () => new ConfigError('Invalid configuration', {
