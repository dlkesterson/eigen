...
import { errorNotifications, notifyError, notifySuccess, notifyWarning, notifyInfo } from '@/lib/error-notifications';
        ...
        const unsubscribe = healthMonitor.subscribe((status) => {
            ...
            status.forEach((currentStatus, name) => {
                ...
                if (previousStatus) {
                    ...
                    if (previousStatus.healthy && !currentStatus.healthy && currentStatus.consecutiveFailures >= 2) {
...
export function useOperationNotifications() {
     ...
     */
    const withNotifications = useCallback(<T extends unknown[], R>(
