    ...
    useEffect(() => {
        ...
        async function setupDeepLinkListener() {
            try {
                ...
                const onOpenUrl = deepLink.onOpenUrl as (callback: (urls: string[]) => void) => Promise<() => void>;

                unlisten = await onOpenUrl((urls: string[]) => {
                    for (const url of urls) {
                        const invitation = parseInviteUrl(url);
                        if (invitation) {
                            logger.info('Received device invitation via deep link', { deviceId: invitation.deviceId });
