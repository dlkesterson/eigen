...
import {
  ...
  type AcceptPendingFolderOptions,
  type FolderType,
...
export type { FolderType, VersioningConfig, AcceptPendingFolderOptions };
...
/**
 * Hook to accept a pending folder share request with advanced options
 * Supports folder type (send/receive mode) and file versioning configuration
 */
export function useAcceptPendingFolder() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (options: AcceptPendingFolderOptions) => {
      await acceptPendingFolder(options);
    },
    onSuccess: () => {
      // Invalidate pending requests and config queries
