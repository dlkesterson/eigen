...
export function useAISearch(options: UseAISearchOptions = {}): UseAISearchReturn {
    ...
    // Sync disabled state - only run when enabled changes
    useEffect(() => {
        const currentStatus = useAIStore.getState().aiStatus;
        if (!enabled && currentStatus !== 'disabled') {
            setStatus('disabled');
            setStatusMessage('AI features are disabled');
            terminateWorker();
        } else if (enabled && currentStatus === 'disabled') {
            setStatus('idle');
            setStatusMessage('');
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [enabled]); // Only depend on enabled, read status from store directly
